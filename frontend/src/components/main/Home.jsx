    ...
    export default function Home() {
        ...
        const selectExam = async (ex) => {
            setLoading(true);
            let allotment = await getAllotment(ex);
            setExam(ex);
            for(let classIdx = 0; classIdx < allotment.length; classIdx++) {
                let place = allotment[classIdx];
                let seats = place.classRoom.seats;
                for(let i = 0; i < seats.length; i++) {
                    for(let j = 0; j < seats[i].length; j++) {
                        if(place.classRoom.seats[i][j].toLowerCase() == user.usn.toLowerCase()) {
                            setAllotment({building: place.building, classRoom: place.classRoom, floor: place.floor, row: i, column: j, classIdx: classIdx});
                            setLoading(false);
                            setClassRoom(seats);
                            return;
                        }
        ...
        return (
            <div className='h-full w-full px-6 py-10 overflow-auto bg-gradient-to-br from-blue-100 text-gray-800'>
                {user && <div className="mb-5 flex flex-col gap-2 justify-center">
                    ...
                    <p className="text-gray-700">Here's your seating information</p>
                </div>}
                ...
                <div className="mb-6 p-5 border border-gray-300 bg-white rounded-2xl flex flex-col gap-3">
                    ...
                    <div>
                        {allExams.length > 0 ? allExams.map((ex, idx) => 
                            <div 
                                key={idx} 
                                className="w-50 mb-2 p-2 rounded-xl border-gray-300 bg-blue-600 text-white flex items-center justify-between"
                                onClick={(e) => selectExam(ex)}
                                > <h1>{ex.examType} - {ex.year}</h1>
                                {exam?._id === ex._id && 
                                <span 
                                    className="material-symbols-outlined"
                ...
                {allotment?.building && <div className="min-h-50 mb-6 p-5 bg-white rounded-2xl text-white flex flex-col gap-3 bg-gradient-to-br from-blue-600 to-purple-600">
                    ...
                    <div className="w-full my-6 flex gap-5">
                        <div className="lg:w-1/2 w-full flex flex-col gap-5">
                            <div className="flex flex-col">
                                <h1 className="text-gray-200">Building</h1>
                                <p className="text-white">{allotment.building}</p>
                            </div>
                            <div>
                                <h1 className="text-gray-200">Floor</h1>
                                <p className="text-white">{allotment.floor}</p>
                            </div>
                        </div>
                        <div className="lg:w-1/2 w-full flex flex-col gap-5">
                            <div>
                                <h1 className="text-gray-200">Class Room</h1>
                                <p className="text-white">{allotment.classRoom.name}</p>
                            </div>
                            <div>
